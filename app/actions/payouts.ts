...
export async function getPayoutStats(userId: string): Promise<PayoutStats> {
  ...
  await redis.set(cacheKey, payoutStats, { ex: 300 });
...
export async function getPayoutHistory(userId: string): Promise<PayoutHistoryItem[]> {
  ...
  await redis.set(cacheKey, formattedHistory, { ex: 600 });
...
export async function requestPayout(userId: string, amount: number) {
  ...
  // ✅ Notify Supabase to trigger real-time updates
  await supabase.from("payout").insert([{ userId: user.id, amount, status: "pending" }]);
